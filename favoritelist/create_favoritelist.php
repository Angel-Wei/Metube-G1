...
<title>MeTube-Create Favorite List</title>
...
<body>
  ...
  <div id="wrapper">
        ...
        <ul class="sidebar-nav nav-pills nav-stacked" id="menu">
            ...
            <li class="active">
              ...
              <ul class="nav-pills nav-stacked" style="list-style-type:none;">
                ...
                <li><a href="view_favoritelist.php">Your Favorite Lists</a></li>
                <!-- Button to create a new favorite list -->
                <li class="active"><a href="create_favoritelist.php">Create New Favorite List</a></li>
              </ul>
            </li>
            <li>
      ...
      <div id="page-content-wrapper">
          <div class="container-fluid xyz">
              <div class="row">
                  <div class="col-lg-12">
                    <form method="post" action="create_favoritelist.php" enctype="multipart/form-data">
                      <h2>Create New Favorite List</h2>

                      <label><b>Please provide supplementary information of the favorite list.</b></label><br>
                      <label for="name">Name:</label>
                      <input type="text" name="name" placeholder="Name your favorite list..."; style="width:400px;"><br>

                      <label for="description">Description of the favorite list(optional):</label><br>
                      <textarea style="font-family: Helvetica;" name="description" rows="4" cols="60" placeholder="Please give a brief description of the favorite list."></textarea>
                      <br>
  ...
  if(isset($_POST['submit'])) {
    // check if a name is given to the favorite list
    if (empty($_POST['name'])){
  		echo "<script type='text/javascript'>alert('Please provied a name to the favorite list');
  		window.location='create_favoritelist.php';</script>";
  	}
    // retrive information from the submitted form data of favorite list
  	$name=$_POST['name'];
    ...
    // check if a favorite list of the same name exists
    $query = "select * from favoritelist where accountid='$accountid' and listname='$name'";
  	$result = mysql_query($query)
  		or die ("Check if a favorite list of the same name exists failed. Could not query the database: <br />". mysql_error());
  	$count = mysql_num_rows($result);

    if($count == 0) // there is no favorite list of the same name that exists
    {
      if(!empty($name) and !empty($description))
      {
        $insertquery = "insert into favoritelist values(NULL, '$accountid', '$name', '$description')";
        $queryresult = mysql_query($insertquery)
            or die("Insert into table favoritelist error by create_favoritelist.php" .mysql_error());
        echo "<script type='text/javascript'>alert('Favorite list created.');
            window.location='create_favoritelist.php';</script>";
      }
      else if(!empty($name) and empty($description))
      {
        $insertquery = "insert into favoritelist values(NULL, '$accountid', '$name', NULL)";
        $queryresult = mysql_query($insertquery)
            or die("Insert into table favoritelist error by create_favoritelist.php" .mysql_error());
        echo "<script type='text/javascript'>alert('Favorite list created.');
            window.location='create_favoritelist.php';</script>";
    ...
    else echo "<script type='text/javascript'>alert('A favorite list of the same name belonging to the same user already exists.');
    window.location='create_favoritelist.php';</script>";
  }
  ?>
  <!-- /#wrapper -->
