<?php
  ...
  // only registered users are able to add media to their Playlists
  ...
  else
  {
    ...
    if($access=="Public")
    {
      ...
      {
        echo "<script type='text/javascript'>alert('Sorry, you are blocked to add this media to the playlist.');
        window.location='../media/media_browse.php';</script>";
      }
    }
...
<title>Add media to playlist</title>
...
<body>
  ...
  <!-- Browse by category -->
  <h4 class="center">Select the playlist of yours to add the media.</h4>
      <?php
      ...
      // if no available playlists
      if($count==0)
      {
      ?>
      <p class="center">No playlists available. <a href="create_playlist.php" target="_blank">Clcik here to create one.</a></p>;
      <?php
      }
      else
      {
?>
        <form class="center" method="post" action="add_to_playlist_process.php?id=<?php echo $_GET['id'];?>" enctype="multipart/form-data">
          <label for="listid">Name of Playlist:</label>
          <select name="listid">
