...
<title>MeTube-View Playlists</title>
...
<body>
  ...
  <div id="wrapper">
          ...
          <li>
            ...
            <li class="active">
              ...
              <ul class="nav-pills nav-stacked" style="list-style-type:none;">
                ...
                <li><a href="create_playlist.php">Create New Playlist</a></li>
              </ul>
            </li>
            <li>
      ...
      <!-- Page Content: display all the playlists created by the current user-->
      <div id="page-content-wrapper">
        <h3>Your playlists:</h3></br>
        <?php
        // retrieve the playlists if any
        $query = "select * from playlist where accountid = '$accountid'";
        $result = mysql_query($query)
      	   or die ("Retrieving playlist information failed. Could not query the database: <br/>". mysql_error());
        $count = mysql_num_rows($result);
        // if no playlists
        if($count == 0)
        {
          echo "<h3 style='background-color:#ffeadb;'>You haven/'t created any playlists.
          <h4><a href='create_playlist.php'>Click here to create playlist.</a></h4></h3>";
        }
        // else: display created playlists
        else
        {
...
?>
            <table class="head" style="margin-left:100px;text-align:'center';" width="70%" cellpadding="0" cellspacing="0">
              <caption><?php echo $result_row[2];?> &nbsp;
                <a href="remove_playlist.php?listid=<?php echo $playlistid;?>" target="_self">Click to delete</a><caption>
              <?php
              if($result_row[3]!='')
              {
              ?>
